...
class ActionSheet extends React.Component {
  ...
  _createButton (title, index) {
    ...
    const lastItemStyle = cancelButtonIndex - 1 === index ? styles.bottomBorder : null
    return (
      <TouchableHighlight
        key={index}
        activeOpacity={1}
        underlayColor={buttonUnderlayColor}
        style={[buttonBoxStyle, lastItemStyle]}
      ...
      >
        <View style={[styles.wrapper]}>
          ...
          >
            {this._renderTitle()}
            {this._renderMessage()}
            <ScrollView style={styles.bottomBorder} scrollEnabled={this.scrollEnabled}>{this._renderOptions()}</ScrollView>
